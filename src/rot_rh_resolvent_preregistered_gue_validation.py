#!/usr/bin/env python3
"""
rot_rh_resolvent_preregistered_gue_validation.py
...
def parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--cache-prefix",
        default="rot_rh_resolvent_preregistered_cache",
    )
    parser.add_argument("--no-cache", action="store_true")

    parser.add_argument(
        "--out-prefix",
        default="rot_rh_resolvent_preregistered_gue_validation",
    )
...
def main() -> int:
    ...
    print("=" * 128)
    print("ROT-RH / PREREGISTERED DEEP GUE VALIDATION")
    print("=" * 128)
    ...
    if spectra is None:
        ...
        for depth in all_depths:
            ...
            spectra[depth] = reconstructed_ordinates(result)

            jacobi_rows.append({
                "depth": depth,
                "node_count": len(spectra[depth]),
                "orthogonality_defect": float(result.orthogonality_defect),
